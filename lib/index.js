...
function bootstrap(server, logger, claudiaApp, routes, options) {
    const handleRequest = makeHandleRequest(logger, claudiaApp, routes, options.lambdaVersion);
...
function runCmd(bootstrapFn) {
    ...
    program
        ...
        .option('--lambdaVersion', 'Specify the lambdaVersion [develop, latest]', 'develop')
        .parse(process.argv);
