...
function getParams(req, routes, lamdaVersion = null) {
...
function makeHandleRequest(logger, app, routes, lamdaVersion) {
    return function (req, res) {
        const params = getParams(req, routes, lamdaVersion);
...
function bootstrap(server, logger, claudiaApp, routes, options) {
    const handleRequest = makeHandleRequest(logger, claudiaApp, routes, options.lamdaVersion);
...
function runCmd(bootstrapFn) {
    ...
    program
        ...
        .option('--lamdaVersion', 'Specify the lamdaVersion [develop, latest]', 'develop')
        .parse(process.argv);
