...
function bootstrap(server, logger, claudiaApp, options) {
    const handleResponse = makeHandleResponse(logger);
...
function runCmd(bootstrapFn) {
    ...
    program
        ...
        .option('-a --app-module <appModule>', 'Specify claudia app path from project root')
        .option('-p --port [port]', `Specify port to use [${config.port}]`, config.port)
        .parse(process.argv);

    const appPath = path.join(process.cwd(), program.appModule);
    const claudiaApp = require(appPath);
